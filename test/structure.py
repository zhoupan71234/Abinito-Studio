    ...
    def get_periodic_positions(self, pos):
        ...
        for i in range(npos):
            ...
            if ntot == 1:
                del_all.append(i)
                pos_new = pos[i]
                pos_new[pos_flag[0]] = 0.0;pos=np.append(pos,[pos_new],axis=0)
                pos_new[pos_flag[0]] = 1.0;pos=np.append(pos,[pos_new],axis=0)
                add_atom = add_atom + 1
            elif ntot == 2:
                del_all.append(i)
                pos_new = pos[i]
                pos_new[pos_flag[0]] = 0.0;pos_new[pos_flag[1]] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[pos_flag[0]] = 0.0;pos_new[pos_flag[1]] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[pos_flag[0]] = 1.0;pos_new[pos_flag[1]] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[pos_flag[0]] = 1.0;pos_new[pos_flag[1]] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                add_atom = add_atom + 3
            elif ntot == 3:
                del_all.append(i)
                pos_new = pos[i]
                pos_new[0] = 0.0;pos_new[1] = 0.0;pos_new[2] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 1.0;pos_new[1] = 0.0;pos_new[2] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 0.0;pos_new[1] = 1.0;pos_new[2] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 0.0;pos_new[1] = 0.0;pos_new[2] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 1.0;pos_new[1] = 1.0;pos_new[2] = 0.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 0.0;pos_new[1] = 1.0;pos_new[2] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 1.0;pos_new[1] = 0.0;pos_new[2] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                pos_new[0] = 1.0;pos_new[1] = 1.0;pos_new[2] = 1.0;
                pos=np.append(pos,[pos_new],axis=0)
                add_atom = add_atom + 7
...
#            typeNo = typeNo + add_atoms
            ...
            for i in range(typeNo):
                ...
                if add_atom == 0:
                    ...
                    sphere_tmp = mlab.points3d(pos_tmp[0][0], pos_tmp[0][1], pos_tmp[0][2],
                    ...
                    pos_tmp = self.poscar.cell.cartesian_positions(pos_tmp1)
                    # for each euqivalent atom
                    sphere_tmp = mlab.points3d(pos_tmp[:, 0], pos_tmp[:, 1], pos_tmp[:, 2],
                                        np.ones(add_atom + 1) * typeRad,
                                        color=typeClr, resolution=60,
                                        scale_factor=1.0,
                                        name="AtomSphere_{}".format(typeName))
